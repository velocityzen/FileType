...
struct FileTypeMatch: Sendable {
    let type: FileTypeMatchType

    let bytesCount: Int?
    let matchString: [String]?
    let matchBytes: [[UInt8]]?
    let match: (@Sendable (Data) -> Bool)?
    let requiresFullFile: Bool
    let minimumPrefixBytes: Int

    init(
        type: FileTypeMatchType,
        bytesCount: Int? = nil,
        matchString: [String]? = nil,
        matchBytes: [[UInt8]]? = nil,
        match: (@Sendable (Data) -> Bool)? = nil,
        requiresFullFile: Bool = false
    ) {
        self.type = type
        ...
        self.match = match
        self.requiresFullFile = requiresFullFile
        minimumPrefixBytes = max(
            bytesCount ?? 0,
            matchBytes?.reduce(into: 0) { currentMaximum, bytes in
                currentMaximum = max(currentMaximum, bytes.count)
            } ?? 0,
            matchString?.reduce(into: 0) { currentMaximum, string in
                currentMaximum = max(currentMaximum, string.count)
            } ?? 0
        )
    }
    ...
    static let all: [FileTypeMatch] = [
        ...
        FileTypeMatch(
            ...
            match: {
                matchPatterns(
                    ...
                    match: [
                        [
                            .byte(0x25), .byte(0x21), .byte(0x50), .byte(0x53), .byte(0x2D),
                            .byte(0x41), .byte(0x64), .byte(0x6F),
                            .byte(0x62), .byte(0x65), .byte(0x2D), .any, .any, .any,
                            .byte(0x20), .byte(0x45), .byte(0x50), .byte(0x53), .byte(0x46),
                            .byte(0x2D),
                        ]
                    ])
            }
        ...
        FileTypeMatch(
            ...
            matchBytes: [
                [
                    0x62, 0x6F, 0x6F, 0x6B, 0x00, 0x00, 0x00, 0x00, 0x6D, 0x61, 0x72, 0x6B, 0x00,
                    0x00, 0x00, 0x00,
                ]
            ]
        ),
        ...
        FileTypeMatch(
            ...
            match: {
                matchPatterns($0, match: [[.byte(0x4C), .byte(0x50)]], offset: 34)
                    && (matchPatterns(
                        $0, match: [[.byte(0x00), .byte(0x00), .byte(0x01)]], offset: 8)
                        || matchPatterns(
                            $0, match: [[.byte(0x01), .byte(0x00), .byte(0x02)]], offset: 8)
                        || matchPatterns(
                            $0, match: [[.byte(0x02), .byte(0x00), .byte(0x02)]], offset: 8))
            }
        ),
