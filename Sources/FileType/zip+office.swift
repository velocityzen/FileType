...
func matchZipHeader(_ data: Data, _ match: (ZipHeader) -> Bool) -> Bool {
    ...
    while data.hasBytes(in: position..<position + 30) {
        guard
            ...
            let filename = data.getString(from: position + 30..<position + 30 + filenameLength)
        else {
            return false
        }
        ...
        if compressedSize == 0 {
            ...
            while position + 30 < data.count, !hasNextHeader {
                if data[position] == 0x50, data[position + 1] == 0x4B, data[position + 2] == 0x03,
                    data[position + 3] == 0x04
                {
                    hasNextHeader = true
                } else {
                    position += 1
...
func matchMSOffice(_ header: ZipHeader, type: String, startsWith: String? = nil) -> Bool {
    ...
    if let startsWith, header.filename.starts(with: startsWith) {
        return true
    }
